...
struct RTLSServer {
  ...
  ruint8 cid_rxlen;
  rboolean cid_offered;                 /* we advertise a connection_id extension */
  ruint8 cid_tx[R_DTLS13_CID_MAX];
  ruint8 cid_txlen;
  rboolean cid_negotiated;              /* CIDs are in use for this connection */
...
r_tls_server_free (RTLSServer * server)
{
  ...
  if (server->rk_read_prev.cipher != NULL)
    r_crypto_cipher_unref (server->rk_read_prev.cipher);
  ...
  r_dtls13_reassembler_free (server->reasm13);
  ...
  r_queue_clear (&server->deferred13, r_buffer_unref);
  r_dtls13_rtx_clear (&server->rtx, server->loop);
...
RTLSError
r_tls_server_set_connection_id (RTLSServer * server, const ruint8 * cid, ruint8 len)
{
  if (R_UNLIKELY (server == NULL)) return R_TLS_ERROR_INVAL;
  if (R_UNLIKELY (server->state > R_TLS_SERVER_HELLO)) return R_TLS_ERROR_WRONG_STATE;
  ...
  /* Opt in to Connection IDs and request that a DTLS 1.3 client tag records it
   * sends us with @cid (RFC 9146). Only then does the server echo the extension
   * and negotiate CIDs. */
  if (len > 0)
    r_memcpy (server->cid_rx, cid, len);
  server->cid_rxlen = len;
  server->cid_offered = TRUE;
  return R_TLS_ERROR_OK;
}

RTLSError
r_tls_server_set_dtls_handshake_fragment (RTLSServer * server, ruint16 size)
{
  if (R_UNLIKELY (server == NULL)) return R_TLS_ERROR_INVAL;
  if (R_UNLIKELY (server->state > R_TLS_SERVER_HELLO)) return R_TLS_ERROR_WRONG_STATE;
...
r_tls_server_nego_hello13 (RTLSServer * server)
{
  ...
  server->rtx.capturing = server->dtls13;

  /* Connection IDs (RFC 9146): if we opted in and the DTLS 1.3 client offered a
   * connection_id, agree -- record the CID the client wants us to tag records
   * with, and echo our own in the ServerHello. */
  if (server->dtls13 && server->cid_offered) {
    RTLSHelloExt cidext = R_TLS_HELLO_EXT_INIT;
    if (r_tls_server_find_ext (server, R_TLS_EXT_TYPE_CONNECTION_ID, &cidext)) {
      ruint8 clen = 0;
      const ruint8 * c = r_tls_hello_ext_connection_id (&cidext, &clen);
      if (clen > R_DTLS13_CID_MAX)
        return R_TLS_ERROR_ILLEGAL_PARAMETER;
      if (clen > 0)
        r_memcpy (server->cid_tx, c, clen);
      server->cid_txlen = clen;
      server->cid_negotiated = TRUE;
    }
  }
  return R_TLS_ERROR_OK;
}
...
/* ServerHello connection_id: the CID the client should tag records to us with
 * (RFC 9146 3). */
static ruint16
r_tls_server_write_ext13_connection_id (RTLSServer * server, ruint8 * p)
{
  r_store_be16 (p, (ruint16) R_TLS_EXT_TYPE_CONNECTION_ID);
  r_store_be16 (p + 2, (ruint16) (1 + server->cid_rxlen));
  p[4] = server->cid_rxlen;
  r_memcpy (p + 5, server->cid_rx, server->cid_rxlen);
  return (ruint16) (5 + server->cid_rxlen);
}
...
r_tls_server_write_hello13 (RTLSServer * server)
{
  ...
  if (r_buffer_map (buf, &info, R_MEM_MAP_WRITE)) {
    ...
    if (server->cid_negotiated)
      extsize += r_tls_server_write_ext13_connection_id (server, ptr + 2 + extsize);
    if (server->resumed13)
    ...
    const ruint8 * plain, rsize plainlen)
{
  ...
  rsize cap = R_TLS_RECORD_HDR_SIZE +
      (server->cid_negotiated ? server->cid_txlen : 0) +
  ...
  if (r_buffer_map (rec, &info, R_MEM_MAP_WRITE)) {
    ...
    if (server->dtls13) {
      ...
      ok = (r_dtls_write_protected_record13 (p, info.size, &reclen,
              &server->rk_write,
              server->cid_negotiated ? server->cid_tx : NULL,
              server->cid_negotiated ? server->cid_txlen : 0,
...
{
  r_dtls13_hs_fold (server->hshash, server->dtls13, msg, len);
}
    ...
    const ruint8 * body, rsize bodylen)
{
  ...
  ruint16 mseq = server->server.msgseq;
  ruint8 * msg;
  RTLSError ret;
  ...
  if (server->dtls13 && server->dtls13_hs_frag > 0 &&
      bodylen > server->dtls13_hs_frag) {
    ...
    rsize cap = server->dtls13_hs_frag, foff;
    ruint8 * frag;
    ...
    const ruint8 * secret, ruint16 epoch)
{
  ...
  if (server->dtls13 && rk == &server->rk_read && rk->cipher != NULL) {
    if (server->rk_read_prev.cipher != NULL)
      r_crypto_cipher_unref (server->rk_read_prev.cipher);
    server->rk_read_prev = *rk;         /* transfers the cipher reference */
    rk->cipher = NULL;
  }
  if (rk->cipher != NULL) {
...
static void
r_tls_server_rtx_fire (rpointer data, REvLoop * loop)
{
  RTLSServer * server = data;
  rsize i, n = r_ptr_array_size (&server->rtx.flight);

  server->rtx.timer = NULL;                 /* one-shot: it has fired */
  if (n == 0)
    return;
  if (server->rtx.tries >= R_DTLS13_RTX_TRIES) {
    if (server->cb.error != NULL)
      server->cb.error (server->userdata, R_TLS_ALERT_TYPE_INTERNAL_ERROR, server);
    r_tls_server_change_state (server, R_TLS_SERVER_ERROR);
    return;
  }
...
r_tls_server_send_out (RTLSServer * server)
{
  ...
  r_dtls13_rtx_arm (&server->rtx, server->loop, r_tls_server_rtx_fire, server);
}

/* ACK the received handshake records so the peer stops retransmitting its final
 * flight (RFC 9147 7). Sent protected under the current write key and not itself
 * retransmitted. */
static void
r_tls_server_send_ack13 (RTLSServer * server)
{
  ruint8 body[sizeof (ruint16) + R_DTLS13_ACK_MAX * 16];
  rsize bodylen = 0;

  if (server->rx_nacks == 0)
    return;
  /* The handshake is complete; stop retransmitting our own flight. */
  server->rtx.capturing = FALSE;
  r_dtls13_rtx_cancel (&server->rtx, server->loop);
  if (r_dtls13_write_ack (body, sizeof (body), &bodylen, server->rx_acks,
          server->rx_nacks) == R_TLS_ERROR_OK)
    r_tls_server_protect_record13 (server, R_TLS_CONTENT_TYPE_ACK, body, bodylen);
  server->rx_nacks = 0;
}

/* Route a DTLS 1.3 handshake record. Plaintext ClientHello records (before read
 * keys) go straight to the state machine; the encrypted flight is reassembled --
 * its fragmented / reordered messages buffered and delivered in message_seq
 * order. */
static RTLSError
r_tls_server_dtls13_handshake (RTLSServer * server, RTLSParser * parser,
    const RTLSServerStateFunc * statefuncs)
{
  RTLSHandshakeType type;
  ...
  if (server->rk_read.cipher == NULL) {
    /* ClientHello (initial or the retry after an HRR): a dedicated state func
     * handles it; note its message_seq so the reassembler starts right after. */
    server->recv_hello_msgseq = mseq;
    r_dtls13_rtx_cancel (&server->rtx, server->loop);         /* a retry acks our HRR */
    do {
  ...
  if (parser->fragment.size < (rsize) R_DTLS_HS_HDR_SIZE + flen)
    return R_TLS_ERROR_CORRUPT_RECORD;
  /* Remember the record number for the ACK we send once the flight completes. */
  if (server->rx_nacks < R_DTLS13_ACK_MAX) {
    server->rx_acks[server->rx_nacks].epoch = server->rk_read.epoch;
    server->rx_acks[server->rx_nacks].seq = parser->seqno;
    server->rx_nacks++;
  }
  if (server->reasm13 == NULL) {
    if ((server->reasm13 = r_dtls13_reassembler_new ()) == NULL)
      return R_TLS_ERROR_OOM;
    server->reasm13->next = (ruint16) (server->recv_hello_msgseq + 1);
  }
  if ((err = r_dtls13_reassembler_push (server->reasm13, (ruint8) type, mseq, len,
  ...
  for (;;) {
    ...
    if ((rmsg = r_dtls13_reassembler_next (server->reasm13, &rlen)) == NULL)
      break;
    r_dtls13_rtx_cancel (&server->rtx, server->loop);         /* progress: our flight was received */
    if ((err = r_dtls_parser_init_handshake13 (&hp, rmsg, rlen)) != R_TLS_ERROR_OK)
      return err;
    do {
      err = statefuncs[server->state] (server, &hp);
    } while (err == R_TLS_ERROR_NOT_NEEDED);
  ...
  /* The client's final flight completed the handshake with no response to send:
   * ACK it so the client stops retransmitting its Finished. */
  if (server->state == R_TLS_SERVER_APPDATA && r_queue_size (&server->qsend) == 0)
    r_tls_server_send_ack13 (server);
  return R_TLS_ERROR_OK;
}
...
static void
r_tls_server_replay_deferred13 (RTLSServer * server)
{
  ...
  if (server->rk_read.cipher == NULL)
    return;
  r_dtls13_take_deferred (&server->deferred13, server->rk_read.epoch, &ready);
  while ((rec = r_queue_pop (&ready)) != NULL) {
    r_tls_server_incoming_data (server, rec);
    r_buffer_unref (rec);
  }
}
...
r_tls_server_incoming_data (RTLSServer * server, RBuffer * buffer)
{
  ...
  if (server->cid_negotiated)
    parser.cidlen = server->cid_rxlen;
  ...
  for (err = r_tls_parser_init_buffer (&parser, server->inbuf);
    ...
    if (server->dtls13) {
      ...
      if (server->rk_read.cipher != NULL &&
         ...
         * cannot key. */
        RTLS13RecordKeys * rk;
        if (parser.epoch == (server->rk_read.epoch & 0x03)) {
          rk = &server->rk_read;
        } else if (server->rk_read_prev.cipher != NULL &&
            parser.epoch == (server->rk_read_prev.epoch & 0x03)) {
          rk = &server->rk_read_prev;
        } else {
          if (parser.epoch == ((server->rk_read.epoch + 1) & 0x03))
            r_dtls13_defer_record (&server->deferred13, &parser);
          continue;
        }
    ...
    if (server->version == R_TLS_VERSION_DTLS_1_3 &&
      ...
      err = r_tls_server_dtls13_handshake (server, &parser, statefuncs);
    } else {
      do {
        err = statefuncs[server->state] (server, &parser);
      } while (err == R_TLS_ERROR_NOT_NEEDED);
  ...
  if (err >= R_TLS_ERROR_OK) {
    r_tls_server_send_out (server);
    r_tls_server_replay_deferred13 (server);
  } else {
