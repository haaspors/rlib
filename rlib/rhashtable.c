...
{
  rsize i, c = R_HASH_TABLE_ALLOC_IDX_TO_SIZE (ht->allocidx);
...
{
  return R_HASH_TABLE_ALLOC_IDX_TO_SIZE (ht->allocidx);
}
...
r_hash_table_insert (RHashTable * ht, rpointer key, rpointer value)
{
  ...
  if (ht->size >= R_HASH_TABLE_ALLOC_IDX_TO_SIZE (ht->allocidx))
