...
static const ruint32 g__r_crc32c_tbl[] = {
...
/* Non-reflected CRC32 table; used by r_crc32bzip2_update. */
static const ruint32 g__r_crc32bzip2_tbl[] = {
...
ruint32
r_crc32_update (ruint32 crc, rconstpointer buffer, rsize size)
{
  const ruint8 * ptr = buffer;

  crc = ~crc;
  while (size--)
    crc = (g__r_crc32_tbl[(crc ^ *ptr++) & 0xff]) ^ (crc >> 8);
  return ~crc;
}
...
ruint32
r_crc32bzip2_update (ruint32 crc, rconstpointer buffer, rsize size)
{
  const ruint8 * ptr = buffer;

  crc = ~crc;
  while (size--)
    crc = (g__r_crc32bzip2_tbl[((crc >> 24) ^ *ptr++) & 0xff]) ^ (crc << 8);
  return ~crc;
}
