...
RTEST (rcryptomac, hmac_md5, R_TEST_TYPE_FAST)
{
  ...
  ruint8 actual[64];
  rsize size = sizeof (actual);
  rchar * tmp;

  r_assert_cmpptr ((hmac = r_hmac_new (R_HASH_TYPE_MD5, "Jefe", 4)), !=, NULL);
  ...
  r_assert (r_hmac_get_data (hmac, actual, &size));
  r_assert_cmpmem (actual, ==, expected, size);
  r_hmac_free (hmac);

  r_assert_cmpptr ((hmac = r_hmac_new (R_HASH_TYPE_MD5, "", 0)), !=, NULL);
...
RTEST (rcryptomac, hmac_sha1, R_TEST_TYPE_FAST)
{
  ...
  r_assert_cmpptr ((hmac = r_hmac_new (R_HASH_TYPE_SHA1, "", 0)), !=, NULL);
  r_assert (r_hmac_update (hmac, "", 0));
  r_assert_cmpstr ((tmp = r_hmac_get_hex (hmac)), ==,
  ...
  r_assert_cmpptr ((hmac = r_hmac_new (R_HASH_TYPE_SHA1, "key", 3)), !=, NULL);
