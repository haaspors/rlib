...
{
  /* r_msg_digest_new returns NULL for an unknown type; r_hmac_new must
   * not crash and must propagate the NULL. */
  RHmac * hmac;

  r_assert_cmpptr (r_hmac_new (R_MSG_DIGEST_TYPE_NONE, "k", 1), ==, NULL);

  /* Every implemented digest backs an HMAC; r_hmac_new should succeed. */
  r_assert_cmpptr ((hmac = r_hmac_new (R_MSG_DIGEST_TYPE_MD2, "k", 1)),
      !=, NULL);
  r_hmac_free (hmac);
  r_assert_cmpptr ((hmac = r_hmac_new (R_MSG_DIGEST_TYPE_MD4, "k", 1)),
      !=, NULL);
  r_hmac_free (hmac);
