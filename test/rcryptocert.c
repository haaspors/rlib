#include <rlib/rcrypto.h>

RTEST (rcryptocert, self_signed_x509v3, RTEST_FAST)
{
  static const ruint8 x509v3[] = {
    
  };
  RCryptoCert * cert;
  RCryptoKey * pk;
  RMsgDigestType signalgo;
  rsize size;

  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, sizeof (x509v3))), !=, NULL);
  r_assert_cmpuint (r_crypto_cert_get_type (cert), ==, R_CRYPTO_CERT_X509);
  r_assert_cmpstr (r_crypto_cert_get_strtype (cert), ==, "X.509");
  r_assert_cmpptr (r_crypto_cert_get_signature (cert, &signalgo, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 1024);
  r_assert_cmpuint (signalgo, ==, R_MSG_DIGEST_TYPE_SHA1);
  r_assert_cmpuint (r_crypto_x509_cert_version (cert), ==, R_X509_VERSION_V3);
  r_assert_cmpuint (r_crypto_x509_cert_serial_number (cert), ==, 17);
  r_assert_cmpstr (r_crypto_x509_cert_issuer (cert),   ==, "CN=Example CA,DC=example,DC=com");
  r_assert_cmpstr (r_crypto_x509_cert_subject (cert),  ==, "CN=Example CA,DC=example,DC=com");
  r_assert_cmpuint (r_crypto_cert_get_valid_from (cert),  ==, r_time_create_unix_time (2004, 4, 30, 14, 25, 34));
  r_assert_cmpuint (r_crypto_cert_get_valid_to (cert),    ==, r_time_create_unix_time (2005, 4, 30, 14, 25, 34));

  r_assert_cmpptr ((pk = r_crypto_cert_get_public_key (cert)), !=, NULL);
  r_assert_cmpuint (r_crypto_key_get_type (pk), ==, R_CRYPTO_PUBLIC_KEY);
  r_assert_cmpuint (r_crypto_key_get_algo (pk), ==, R_CRYPTO_ALGO_RSA);
  r_assert_cmpuint (r_crypto_key_get_bitsize (pk), ==, 1024);
  r_crypto_key_unref (pk);

  /* BIT STRING bytes 0x03 0x02 0x01 0x06: unused=1, content=0x06.
   * Per RFC 5280 / X.690 that's bits 5 and 6 (counted from the MSB)
   * = keyCertSign | cRLSign, the usual CA-cert combination. */
  r_assert_cmphex (r_crypto_x509_cert_key_usage (cert), ==,
      R_X509_KEY_USAGE_KEY_CERT_SIGN | R_X509_KEY_USAGE_CRL_SIGN);
  r_assert_cmphex (r_crypto_x509_cert_ext_key_usage (cert), ==,
      R_X509_EXT_KEY_USAGE_NONE);
  r_assert_cmpptr (r_crypto_x509_cert_issuer_unique_id (cert, NULL), ==, NULL);
  r_assert_cmpptr (r_crypto_x509_cert_subject_unique_id (cert, NULL), ==, NULL);
  r_assert_cmpptr (r_crypto_x509_cert_subject_key_id (cert, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 20);

  r_assert (r_crypto_x509_cert_is_ca (cert));
  r_assert (r_crypto_x509_cert_is_self_issued (cert));
  r_assert (r_crypto_x509_cert_is_self_signed (cert));

  /* This is just for fun. Basically SubjectKeyIdentifier is what RFC5280 discribes;
   *  (1) The keyIdentifier is composed of the 160-bit SHA-1 hash of the
   *       value of the BIT STRING subjectPublicKey (excluding the tag,
   *       length, and number of unused bits).
   */
  {
    RMsgDigest * md = r_sha1_new ();
    ruint8 sha1[20];

    r_assert (r_msg_digest_update (md, x509v3 + 223, 140));
    r_assert (r_msg_digest_get_data (md, sha1, sizeof (sha1), &size));
    r_assert_cmpmem (r_crypto_x509_cert_subject_key_id (cert, NULL), ==, sha1, size);

    r_msg_digest_free (md);
  }

  r_assert_cmpuint (r_crypto_x509_cert_verify_signature (cert, cert), ==, R_CRYPTO_OK);

  r_crypto_cert_unref (cert);
}
RTEST_END;

RTEST (rcryptocert, valid_x509v3, RTEST_FAST)
{
  static const ruint8 x509v3[] = {
    
  };
  RCryptoCert * cert;
  RCryptoKey * pk;
  RMsgDigestType signalgo;
  rsize size;


  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, sizeof (x509v3))), !=, NULL);
  r_assert_cmpuint (r_crypto_cert_get_type (cert), ==, R_CRYPTO_CERT_X509);
  r_assert_cmpstr (r_crypto_cert_get_strtype (cert), ==, "X.509");
  r_assert_cmpptr (r_crypto_cert_get_signature (cert, &signalgo, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 2048);
  r_assert_cmpuint (signalgo, ==, R_MSG_DIGEST_TYPE_SHA256);
  r_assert_cmpuint (r_crypto_x509_cert_version (cert), ==, R_X509_VERSION_V3);
  r_assert_cmpuint (r_crypto_x509_cert_serial_number (cert), ==, 1);
  r_assert_cmpstr (r_crypto_x509_cert_issuer (cert),   ==, "CN=Good CA,O=Test Certificates 2011,C=US");
  r_assert_cmpstr (r_crypto_x509_cert_subject (cert),  ==, "CN=Valid EE Certificate Test1,O=Test Certificates 2011,C=US");
  r_assert_cmpuint (r_crypto_cert_get_valid_from (cert),  ==, r_time_create_unix_time (2010,  1,  1, 8, 30, 0));
  r_assert_cmpuint (r_crypto_cert_get_valid_to (cert),    ==, r_time_create_unix_time (2030, 12, 31, 8, 30, 0));

  r_assert_cmpptr ((pk = r_crypto_cert_get_public_key (cert)), !=, NULL);
  r_assert_cmpuint (r_crypto_key_get_type (pk), ==, R_CRYPTO_PUBLIC_KEY);
  r_assert_cmpuint (r_crypto_key_get_algo (pk), ==, R_CRYPTO_ALGO_RSA);
  r_assert_cmpuint (r_crypto_key_get_bitsize (pk), ==, 2048);
  r_crypto_key_unref (pk);

  r_assert_cmphex (r_crypto_x509_cert_key_usage (cert), ==,
      R_X509_KEY_USAGE_DIGITAL_SIGNATURE | R_X509_KEY_USAGE_NON_REPUDIATION |
      R_X509_KEY_USAGE_KEY_ENCIPHERMENT  | R_X509_KEY_USAGE_DATA_ENCIPHERMENT);
  r_assert_cmpptr (r_crypto_x509_cert_issuer_unique_id (cert, NULL), ==, NULL);
  r_assert_cmpptr (r_crypto_x509_cert_subject_unique_id (cert, NULL), ==, NULL);
  r_assert_cmpptr (r_crypto_x509_cert_subject_key_id (cert, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 20);
  r_assert_cmpptr (r_crypto_x509_cert_authority_key_id (cert, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 20);
  r_assert (r_crypto_x509_cert_has_policy (cert, "2.16.840.1.101.3.2.1.48.1")); /* nistTestPolicy1 */

  r_assert (!r_crypto_x509_cert_is_ca (cert));
  r_assert (!r_crypto_x509_cert_is_self_issued (cert));
  r_assert (!r_crypto_x509_cert_is_self_signed (cert));

  r_assert_cmpuint (r_crypto_x509_cert_verify_signature (cert, cert), ==, R_CRYPTO_VERIFY_FAILED);

  /* Round-trip via the encoder: re-export the cert, then parse the
   * exported bytes back. Verifies the certificatePolicies extension
   * write path emits a structure the reader can parse. */
  {
    RAsn1BinEncoder * enc;
    ruint8 * round = NULL;
    rsize roundsize = 0;
    RCryptoCert * cert2;

    r_assert_cmpptr ((enc = r_asn1_bin_encoder_new (R_ASN1_DER)), !=, NULL);
    r_assert_cmpint (r_crypto_cert_export (cert, enc), ==, R_CRYPTO_OK);
    r_assert_cmpptr ((round = r_asn1_bin_encoder_get_data (enc, &roundsize)),
        !=, NULL);
    r_asn1_bin_encoder_unref (enc);

    r_assert_cmpptr ((cert2 = r_crypto_x509_cert_new_take (round, roundsize)),
        !=, NULL);
    r_assert (r_crypto_x509_cert_has_policy (cert2, "2.16.840.1.101.3.2.1.48.1"));
    r_crypto_cert_unref (cert2);
  }

  r_crypto_cert_unref (cert);
}
RTEST_END;


RTEST (rcryptocert, openssl_gen_x509_rsa_1024, RTEST_FAST)
{
  static const ruint8 x509v3[] = {
    
  };
  RCryptoCert * cert;
  RCryptoKey * pk;
  RMsgDigestType signalgo;
  rsize size;

  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, sizeof (x509v3))), !=, NULL);
  r_assert_cmpuint (r_crypto_cert_get_type (cert), ==, R_CRYPTO_CERT_X509);
  r_assert_cmpstr (r_crypto_cert_get_strtype (cert), ==, "X.509");
  r_assert_cmpptr (r_crypto_cert_get_signature (cert, &signalgo, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 1024);
  r_assert_cmpuint (signalgo, ==, R_MSG_DIGEST_TYPE_SHA1);
  r_assert_cmpuint (r_crypto_x509_cert_version (cert), ==, R_X509_VERSION_V3);
  r_assert_cmpuint (r_crypto_x509_cert_serial_number (cert), ==, RUINT64_CONSTANT (9471694375470879023));
  r_assert_cmpstr (r_crypto_x509_cert_issuer (cert),   ==, "CN=ieei,O=82 bits,ST=Some-State,C=NO");
  r_assert_cmpstr (r_crypto_x509_cert_subject (cert),  ==, "CN=ieei,O=82 bits,ST=Some-State,C=NO");
  r_assert_cmpuint (r_crypto_cert_get_valid_from (cert),  ==, r_time_create_unix_time (2016, 10, 5, 18, 5, 46));
  r_assert_cmpuint (r_crypto_cert_get_valid_to (cert),    ==, r_time_create_unix_time (2016, 11, 4, 18, 5, 46));

  r_assert_cmpptr ((pk = r_crypto_cert_get_public_key (cert)), !=, NULL);
  r_assert_cmpuint (r_crypto_key_get_type (pk), ==, R_CRYPTO_PUBLIC_KEY);
  r_assert_cmpuint (r_crypto_key_get_algo (pk), ==, R_CRYPTO_ALGO_RSA);
  r_assert_cmpuint (r_crypto_key_get_bitsize (pk), ==, 1024);
  r_crypto_key_unref (pk);

  r_assert (r_crypto_x509_cert_is_ca (cert));
  r_assert (r_crypto_x509_cert_is_self_issued (cert));
  r_assert (r_crypto_x509_cert_is_self_signed (cert));

  r_assert_cmpuint (r_crypto_x509_cert_verify_signature (cert, cert), ==, R_CRYPTO_OK);

  r_crypto_cert_unref (cert);
}
RTEST_END;

RTEST (rcryptocert, nodejs_pem_gen_x509_self_signed_rsa_2048_days_1, RTEST_FAST)
{
  static const rchar * x509_base64 =
    "MIICpDCCAYwCCQDz+4pqXkrUmjANBgkqhkiG9w0BAQsFADAUMRIwEAYDVQQDEwls"
    "b2NhbGhvc3QwHhcNMTYxMDA1MjEwMzU1WhcNMTYxMDA2MjEwMzU1WjAUMRIwEAYD"
    "VQQDEwlsb2NhbGhvc3QwggEiMA0GCSqGSIb3DQEBAQUAA4IBDwAwggEKAoIBAQDR"
    "TOEwF+M6sw5AKOTf8ID4douHxYmhhZ/b8y7WzoTUTOh76ZzEO3FKYBK7zDwV2TC1"
    "p1nazr8vHpR773UdST6dsVzU+6EGRrzOUVq88shcA9uMJ6xK2TzbbXogyBXvsKtR"
    "8x0TbCY8r74nT8OGjKTSgvTQtaPJZNw0BN8owUhbX3MHcBBwzbWWzbsj6GvOWLrS"
    "tT8pyw1DHMvz4OlEE2Q0h+BCk2ikgyedSRTfK05syVwkoVk2VinxNa/adIqW4jHX"
    "/bSlCemhWb/FCEsdnmKNhB5OF9KJnooOpXRPemRG+WdVYtKcV67GfTI5aGO//6yY"
    "GKfQzLmsufvyycky/w1fAgMBAAEwDQYJKoZIhvcNAQELBQADggEBACMVM7zZxdxn"
    "X+cWml5gErov30W4LoeGM4R3/kI6kXke+HI86f6tPqIjRualHlxGy//QZgS7bZZJ"
    "QTz8emBzY4ZyP6b3wSKdHdJ0u+DKQ5gv3bK8sGw7IN08xSRGx0snYWbSrWhTHZTJ"
    "H8z1fkHNDdAFcCsvWSF9S/WiTDyJw1v4Lcxr+2nZxpUOWEfSE6r7rDSIs93fGlG4"
    "XozSfCUV2AQAO9gcQYrwG7nh2b4uzfirF3Srh+3l3upw8MpNsZzeaEhb/wvZqwHY"
    "jZUavcNP8pWiVYmNaGnP9jL24wZpv+gExknNFMheiFiSwMFFLNY81TSFhFG0n4Zw"
    "pKbc1ltQGjw=";
  rsize size;
  ruint8 * x509v3;
  RCryptoCert * cert;
  RCryptoKey * pk;
  RMsgDigestType signalgo;

  r_assert_cmpptr ((x509v3 = r_base64_decode_dup (x509_base64, -1, &size)), !=, NULL);

  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, size)), !=, NULL);
  r_free (x509v3);

  r_assert_cmpuint (r_crypto_cert_get_type (cert), ==, R_CRYPTO_CERT_X509);
  r_assert_cmpstr (r_crypto_cert_get_strtype (cert), ==, "X.509");
  r_assert_cmpptr (r_crypto_cert_get_signature (cert, &signalgo, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 2048);
  r_assert_cmpuint (signalgo, ==, R_MSG_DIGEST_TYPE_SHA256);
  r_assert_cmpuint (r_crypto_x509_cert_version (cert), ==, R_X509_VERSION_V1);
  r_assert_cmpuint (r_crypto_x509_cert_serial_number (cert), ==, RUINT64_CONSTANT (17580797759823991962));
  r_assert_cmpstr (r_crypto_x509_cert_issuer (cert),   ==, "CN=localhost");
  r_assert_cmpstr (r_crypto_x509_cert_subject (cert),  ==, "CN=localhost");
  r_assert_cmpuint (r_crypto_cert_get_valid_from (cert),  ==, r_time_create_unix_time (2016, 10, 5, 21, 3, 55));
  r_assert_cmpuint (r_crypto_cert_get_valid_to (cert),    ==, r_time_create_unix_time (2016, 10, 6, 21, 3, 55));

  r_assert_cmpptr ((pk = r_crypto_cert_get_public_key (cert)), !=, NULL);
  r_assert_cmpuint (r_crypto_key_get_type (pk), ==, R_CRYPTO_PUBLIC_KEY);
  r_assert_cmpuint (r_crypto_key_get_algo (pk), ==, R_CRYPTO_ALGO_RSA);
  r_assert_cmpuint (r_crypto_key_get_bitsize (pk), ==, 2048);
  r_crypto_key_unref (pk);

  r_assert (!r_crypto_x509_cert_is_ca (cert));
  r_assert (r_crypto_x509_cert_is_self_issued (cert));
  r_assert (r_crypto_x509_cert_is_self_signed (cert));

  r_assert_cmpuint (r_crypto_x509_cert_verify_signature (cert, cert), ==, R_CRYPTO_OK);

  r_crypto_cert_unref (cert);
}
RTEST_END;

RTEST (rcryptocert, chrome_ec_secp256r1_dtls, RTEST_FAST)
{
  static const ruint8 x509v3[] = {
    
  };

  RCryptoCert * cert;
  RCryptoKey * pk;
  RMsgDigestType signalgo;
  rsize size;

  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, sizeof (x509v3))), !=, NULL);
  r_assert_cmpuint (r_crypto_cert_get_type (cert), ==, R_CRYPTO_CERT_X509);
  r_assert_cmpstr (r_crypto_cert_get_strtype (cert), ==, "X.509");
  r_assert_cmpptr (r_crypto_cert_get_signature (cert, &signalgo, &size), !=, NULL);
  r_assert_cmpuint (size, ==, 568);
  r_assert_cmpuint (signalgo, ==, R_MSG_DIGEST_TYPE_SHA256);
  r_assert_cmpuint (r_crypto_x509_cert_version (cert), ==, R_X509_VERSION_V3);
  r_assert_cmpuint (r_crypto_x509_cert_serial_number (cert), ==, RUINT64_CONSTANT (10701905757333968641));
  r_assert_cmpstr (r_crypto_x509_cert_issuer (cert),   ==, "CN=WebRTC");
  r_assert_cmpstr (r_crypto_x509_cert_subject (cert),  ==, "CN=WebRTC");
  r_assert_cmpuint (r_crypto_cert_get_valid_from (cert),  ==, r_time_create_unix_time (2016, 10, 11,  7, 28, 11));
  r_assert_cmpuint (r_crypto_cert_get_valid_to (cert),    ==, r_time_create_unix_time (2016, 11, 11,  7, 28, 11));

  r_assert_cmpptr ((pk = r_crypto_cert_get_public_key (cert)), !=, NULL);
  r_assert_cmpuint (r_crypto_key_get_type (pk), ==, R_CRYPTO_PUBLIC_KEY);
  r_assert_cmpuint (r_crypto_key_get_algo (pk), ==, R_CRYPTO_ALGO_ECDSA);
  r_assert_cmpuint (r_crypto_key_get_bitsize (pk), ==, 65*8);
  r_assert_cmphex (r_ecc_key_get_curve (pk), ==, R_ECURVE_ID_SECP256R1);
  r_crypto_key_unref (pk);

  r_crypto_cert_unref (cert);
}
RTEST_END;

RTEST (rcryptocert, export_and_fingerprint, RTEST_FAST)
{
  static const rchar * x509_base64 =
    "MIIC8TCCAdmgAwIBAgIJALoi/+XOQDHjMA0GCSqGSIb3DQEBCwUAMA8xDTALBgNV"
    "BAMMBHJsaWIwHhcNMTYxMTE1MTMzNjI0WhcNMTcxMTE1MTMzNjI0WjAPMQ0wCwYD"
    "VQQDDARybGliMIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAwjolUmQU"
    "r9Q2FZ7O3qau+Z6+VvuJROvxzjt1aIQLLO/hF0Ya56BZCZD5aKyqQM//fTm97VTb"
    "CQYBaNg03D20XPDIWmr7EdHxYK+YI+jz7DrWqhM4jwSvvteXXXWD7bVdCq+RyveD"
    "NrgoGZqL5UCiWS1BWkB9nS/KQtgxrT3hWSOlG1xRh6hfeIy4H2CB3Qk/Q3PHjMcH"
    "7CKhCj+ctbqR3r2K3BLL3fgZKnfQdCPsZplN8Ey4hSOc/67NQK/yn/S0JgeHmjb8"
    "D5xbaDiOloOHJJg6dm1QU0UuEpiK2Uda0VR6TGu9Ci05h5U3HoV9CbyAGQhmFSem"
    "NreAELYv89sMgwIDAQABo1AwTjAdBgNVHQ4EFgQUXFVr3x4Bcglp/MP0ZFEk/Ntz"
    "wJYwHwYDVR0jBBgwFoAUXFVr3x4Bcglp/MP0ZFEk/NtzwJYwDAYDVR0TBAUwAwEB"
    "/zANBgkqhkiG9w0BAQsFAAOCAQEAL4ZKyDRXP3+Jr/GN+p6WbFW3tHuhxWxy8rMy"
    "W7OHX/sHASzJiaEmjtIlPx/7uFFowktEmXyybEmBvYp64UZ2mo2v+CCm+236wPTS"
    "gGfpcp9nP2RI0VFdJLHuqWapa5CQJZISRAO/tj7UqflOWBohm04EvmJe53JGEq+4"
    "Dk41kC+z3jVPGHG+jR3uYOw7JCmFT+bt4P5EDxGAKe9eoweLHBJ8vlJ7cUdHhBv1"
    "BUCMVR86kPZFzHKVQtWNXt26H/khgz7RA/qUSJA17Nk2h0h60b1AbkljkduWWIMZ"
    "5B2DUz4MEDUHjppHF9+A2q5ZN+25eOYbrkS5Dq50VPNrvd8dSQ==";
  rsize sizein, sizeout;
  ruint8 * x509v3in, * x509v3out;
  RCryptoCert * cert;
  RAsn1BinEncoder * enc;
  rchar * fingerprint;

  r_assert_cmpptr ((x509v3in = r_base64_decode_dup (x509_base64, -1, &sizein)), !=, NULL);
  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3in, sizein)), !=, NULL);

  r_assert_cmpptr ((enc = r_asn1_bin_encoder_new (R_ASN1_DER)), !=, NULL);
  r_assert_cmpint (r_crypto_cert_export (cert, enc), ==, R_CRYPTO_OK);

  r_assert_cmpptr ((x509v3out = r_asn1_bin_encoder_get_data (enc, &sizeout)), !=, NULL);
  r_asn1_bin_encoder_unref (enc);

  r_assert_cmpuint (sizein, ==, sizeout);
  r_assert_cmpmem (x509v3in, ==, x509v3out, sizeout);

  r_free (x509v3out);
  r_free (x509v3in);

  r_assert_cmpstr ((fingerprint = r_crypto_cert_fingerprint_str (cert, R_MSG_DIGEST_TYPE_SHA256, ":", 1)),
      ==, "74:ac:bd:23:74:28:88:33:67:89:4a:3e:05:12:24:55:c1:1c:71:c4:de:00:c7:47:d3:5d:98:51:36:a8:0a:b8");
  r_free (fingerprint);

  r_crypto_cert_unref (cert);
}
RTEST_END;

/* Self-signed RSA-1024 cert carrying a multi-valued SubjectAltName
 * (DNS x2, e-mail, URI, IPv4, IPv6), a three-purpose ExtendedKeyUsage
 * and a two-OID certificatePolicies. Generated with:
 *   openssl req -new -x509 -key k.pem -outform DER -config <cfg> with
 *   subjectAltName=DNS:a.example.com,DNS:b.example.com,email:foo@example.com,
 *     URI:https://example.com/p,IP:192.0.2.1,IP:2001:db8::1
 *   extendedKeyUsage=serverAuth,clientAuth,codeSigning
 *   certificatePolicies=1.2.3.4,1.2.3.5
 * The multi-valued EKU/policies also exercise the SEQUENCE OF iteration. */
RTEST (rcryptocert, x509_subject_alt_name, RTEST_FAST)
{
  static const ruint8 x509v3[] = {
    ,
  };
  RCryptoCert * cert;
  const RX509GeneralName * gn;
  const ruint8 * ip;
  rsize ipsize;

  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, sizeof (x509v3))), !=, NULL);

  r_assert_cmpuint (r_crypto_x509_cert_subject_alt_name_count (cert), ==, 7);
  r_assert_cmpptr ((gn = r_crypto_x509_cert_subject_alt_name (cert, 0)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_DNS);
  r_assert_cmpstr (r_x509_general_name_as_string (gn), ==, "a.example.com");
  r_assert_cmpstr (r_x509_general_name_as_string (
        r_crypto_x509_cert_subject_alt_name (cert, 1)), ==, "b.example.com");
  r_assert_cmpptr ((gn = r_crypto_x509_cert_subject_alt_name (cert, 2)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_RFC822);
  r_assert_cmpstr (r_x509_general_name_as_string (gn), ==, "foo@example.com");
  r_assert_cmpptr ((gn = r_crypto_x509_cert_subject_alt_name (cert, 3)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_URI);
  r_assert_cmpstr (r_x509_general_name_as_string (gn), ==, "https://example.com/p");
  r_assert_cmpptr ((gn = r_crypto_x509_cert_subject_alt_name (cert, 4)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_IP);
  r_assert_cmpptr ((ip = r_x509_general_name_as_ip (gn, &ipsize)), !=, NULL);
  r_assert_cmpuint (ipsize, ==, 4);
  r_assert_cmpmem (ip, ==, "\xc0\x00\x02\x01", 4);
  r_assert_cmpptr ((gn = r_crypto_x509_cert_subject_alt_name (cert, 5)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_IP);
  r_assert_cmpptr (r_x509_general_name_as_ip (gn, &ipsize), !=, NULL);
  r_assert_cmpuint (ipsize, ==, 16);
  /* Type-mismatched accessors return NULL. */
  r_assert_cmpptr (r_x509_general_name_as_string (gn), ==, NULL);
  r_assert_cmpptr ((gn = r_crypto_x509_cert_subject_alt_name (cert, 6)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_REGISTERED_ID);
  {
    rchar * oid = r_x509_general_name_as_oid (gn);
    r_assert_cmpstr (oid, ==, "1.2.3.4");
    r_free (oid);
  }
  r_assert_cmpptr (r_crypto_x509_cert_subject_alt_name (cert, 7), ==, NULL);

  /* Multi-valued EKU and certificatePolicies (iteration fix). */
  r_assert_cmphex (r_crypto_x509_cert_ext_key_usage (cert), ==,
      R_X509_EXT_KEY_USAGE_SERVER_AUTH | R_X509_EXT_KEY_USAGE_CLIENT_AUTH |
      R_X509_EXT_KEY_USAGE_CODE_SIGNING);
  r_assert (r_crypto_x509_cert_has_policy (cert, "1.2.3.4"));
  r_assert (r_crypto_x509_cert_has_policy (cert, "1.2.3.5"));

  /* Round-trip: re-export, re-parse, re-assert the modeled fields. */
  {
    RAsn1BinEncoder * enc;
    ruint8 * round = NULL;
    rsize roundsize = 0;
    RCryptoCert * c2;

    r_assert_cmpptr ((enc = r_asn1_bin_encoder_new (R_ASN1_DER)), !=, NULL);
    r_assert_cmpint (r_crypto_cert_export (cert, enc), ==, R_CRYPTO_OK);
    r_assert_cmpptr ((round = r_asn1_bin_encoder_get_data (enc, &roundsize)), !=, NULL);
    r_asn1_bin_encoder_unref (enc);

    r_assert_cmpptr ((c2 = r_crypto_x509_cert_new_take (round, roundsize)), !=, NULL);
    r_assert_cmpuint (r_crypto_x509_cert_subject_alt_name_count (c2), ==, 7);
    r_assert_cmpstr (r_x509_general_name_as_string (
          r_crypto_x509_cert_subject_alt_name (c2, 2)), ==, "foo@example.com");
    r_assert_cmpstr (r_x509_general_name_as_string (
          r_crypto_x509_cert_subject_alt_name (c2, 3)), ==, "https://example.com/p");
    r_assert_cmpuint (r_x509_general_name_type (
          r_crypto_x509_cert_subject_alt_name (c2, 6)), ==, R_X509_GENERAL_NAME_REGISTERED_ID);
    r_assert_cmphex (r_crypto_x509_cert_ext_key_usage (c2), ==,
        R_X509_EXT_KEY_USAGE_SERVER_AUTH | R_X509_EXT_KEY_USAGE_CLIENT_AUTH |
        R_X509_EXT_KEY_USAGE_CODE_SIGNING);
    r_assert (r_crypto_x509_cert_has_policy (c2, "1.2.3.4"));
    r_assert (r_crypto_x509_cert_has_policy (c2, "1.2.3.5"));
    r_crypto_cert_unref (c2);
  }

  r_crypto_cert_unref (cert);
}
RTEST_END;

/* Self-signed RSA-1024 CA cert carrying NameConstraints (permitted
 * DNS + IP, excluded e-mail), PolicyMappings (two OID pairs) and an
 * AuthorityKeyIdentifier with authorityCertIssuer (directoryName).
 * Generated with openssl req -new -x509 and:
 *   nameConstraints=critical,permitted;DNS:.example.com,
 *     permitted;IP:192.0.2.0/255.255.255.0,excluded;email:.evil.example
 *   policyMappings=1.2.3.4:1.2.3.5,1.2.3.6:1.2.3.7
 *   authorityKeyIdentifier=keyid:always,issuer:always */
RTEST (rcryptocert, x509_name_constraints_policy_mappings, RTEST_FAST)
{
  static const ruint8 x509v3[] = {
    ,
  };
  RCryptoCert * cert;
  const RX509GeneralName * gn;
  const rchar * iss, * subj;
  rsize ipsize;

  r_assert_cmpptr ((cert = r_crypto_x509_cert_new (x509v3, sizeof (x509v3))), !=, NULL);
  r_assert (r_crypto_x509_cert_is_ca (cert));

  /* NameConstraints. */
  r_assert_cmpuint (r_crypto_x509_cert_name_constraint_permitted_count (cert), ==, 2);
  r_assert_cmpptr ((gn = r_crypto_x509_cert_name_constraint_permitted (cert, 0)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_DNS);
  r_assert_cmpstr (r_x509_general_name_as_string (gn), ==, ".example.com");
  r_assert_cmpptr ((gn = r_crypto_x509_cert_name_constraint_permitted (cert, 1)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_IP);
  r_assert_cmpptr (r_x509_general_name_as_ip (gn, &ipsize), !=, NULL);
  r_assert_cmpuint (ipsize, ==, 8);  /* CIDR: address + mask */
  r_assert_cmpuint (r_crypto_x509_cert_name_constraint_excluded_count (cert), ==, 1);
  r_assert_cmpptr ((gn = r_crypto_x509_cert_name_constraint_excluded (cert, 0)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_RFC822);
  r_assert_cmpstr (r_x509_general_name_as_string (gn), ==, ".evil.example");

  /* PolicyMappings. */
  r_assert_cmpuint (r_crypto_x509_cert_policy_mapping_count (cert), ==, 2);
  r_assert (r_crypto_x509_cert_policy_mapping (cert, 0, &iss, &subj));
  r_assert_cmpstr (iss, ==, "1.2.3.4");
  r_assert_cmpstr (subj, ==, "1.2.3.5");
  r_assert (r_crypto_x509_cert_policy_mapping (cert, 1, &iss, &subj));
  r_assert_cmpstr (iss, ==, "1.2.3.6");
  r_assert_cmpstr (subj, ==, "1.2.3.7");
  r_assert (!r_crypto_x509_cert_policy_mapping (cert, 2, &iss, &subj));

  /* AuthorityKeyIdentifier authorityCertIssuer (directoryName). */
  r_assert_cmpuint (r_crypto_x509_cert_authority_cert_issuer_count (cert), ==, 1);
  r_assert_cmpptr ((gn = r_crypto_x509_cert_authority_cert_issuer (cert, 0)), !=, NULL);
  r_assert_cmpuint (r_x509_general_name_type (gn), ==, R_X509_GENERAL_NAME_DIRECTORY);
  r_assert_cmpstr (r_x509_general_name_as_dn (gn), ==, r_crypto_x509_cert_issuer (cert));

  /* Round-trip. */
  {
    RAsn1BinEncoder * enc;
    ruint8 * round = NULL;
    rsize roundsize = 0;
    RCryptoCert * c2;

    r_assert_cmpptr ((enc = r_asn1_bin_encoder_new (R_ASN1_DER)), !=, NULL);
    r_assert_cmpint (r_crypto_cert_export (cert, enc), ==, R_CRYPTO_OK);
    r_assert_cmpptr ((round = r_asn1_bin_encoder_get_data (enc, &roundsize)), !=, NULL);
    r_asn1_bin_encoder_unref (enc);

    r_assert_cmpptr ((c2 = r_crypto_x509_cert_new_take (round, roundsize)), !=, NULL);
    r_assert_cmpuint (r_crypto_x509_cert_name_constraint_permitted_count (c2), ==, 2);
    r_assert_cmpstr (r_x509_general_name_as_string (
          r_crypto_x509_cert_name_constraint_permitted (c2, 0)), ==, ".example.com");
    r_assert_cmpuint (r_crypto_x509_cert_name_constraint_excluded_count (c2), ==, 1);
    r_assert_cmpstr (r_x509_general_name_as_string (
          r_crypto_x509_cert_name_constraint_excluded (c2, 0)), ==, ".evil.example");
    r_assert_cmpuint (r_crypto_x509_cert_policy_mapping_count (c2), ==, 2);
    r_assert (r_crypto_x509_cert_policy_mapping (c2, 1, &iss, &subj));
    r_assert_cmpstr (iss, ==, "1.2.3.6");
    r_assert_cmpstr (subj, ==, "1.2.3.7");
    r_assert_cmpuint (r_crypto_x509_cert_authority_cert_issuer_count (c2), ==, 1);
    r_assert_cmpptr ((gn = r_crypto_x509_cert_authority_cert_issuer (c2, 0)), !=, NULL);
    r_assert_cmpstr (r_x509_general_name_as_dn (gn), ==, r_crypto_x509_cert_issuer (c2));
    r_crypto_cert_unref (c2);
  }

  r_crypto_cert_unref (cert);
}
RTEST_END;

