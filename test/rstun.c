...
/* Valid header (msglen 8) but the single attribute claims a 0xffff-byte
 * value that doesn't fit -- must be rejected by the TLV iterator. */
static const ruint8 pkt_oversized_attr[] = {
  0x00, 0x01, 0x00, 0x08, 0x21, 0x12, 0xa4, 0x42, 0x46, 0x76, 0x41, 0x31, 0x65, 0x6d, 0x75, 0x49,
  0x73, 0x6b, 0x4e, 0x59, 0x00, 0x01, 0xff, 0xff, 0x00, 0x00, 0x00, 0x00
};
...
RTEST (rstun, oversized_attribute, RTEST_FAST)
{
  RStunAttrTLV tlv;

  /* Header validates, but the attribute's declared value length doesn't
   * fit in the message; the TLV iterator must reject it rather than let
   * the parse helpers read past the buffer. */
  r_assert (r_stun_is_valid_msg (pkt_oversized_attr, sizeof (pkt_oversized_attr)));
  r_assert (!r_stun_attr_tlv_first (pkt_oversized_attr, &tlv));
}
RTEST_END;
