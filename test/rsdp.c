...
RTEST (rsdp, from_rfc, RTEST_FAST)
{
  ...
  r_assert_cmpuint (r_sdp_buf_timing_count (&sdp), ==, 1);
  r_assert_cmpstr ((tmp = r_sdp_buf_timing_start (&sdp, 0)), ==, "2873397496"); r_free (tmp);
  r_assert_cmpstr ((tmp = r_sdp_buf_timing_stop (&sdp, 0)), ==, "2873404696"); r_free (tmp);
  r_assert_cmpuint (r_sdp_buf_timing_repeat_count (&sdp, 0), ==, 0);
...
RTEST (rsdp, chrome_webrtc_offer, RTEST_FAST)
{
  ...
  r_assert_cmpuint (r_sdp_buf_bandwidth_count (&sdp), ==, 0);
  r_assert_cmpuint (r_sdp_buf_timing_count (&sdp), ==, 1);
  r_assert_cmpstr ((tmp = r_sdp_buf_timing_start (&sdp, 0)), ==, "0"); r_free (tmp);
  r_assert_cmpstr ((tmp = r_sdp_buf_timing_stop (&sdp, 0)), ==, "0"); r_free (tmp);
  r_assert_cmpuint (r_sdp_buf_timing_repeat_count (&sdp, 0), ==, 0);
  r_assert_cmpuint (r_sdp_buf_time_zone_count (&sdp), ==, 0);
